...
/*
 ...
 Copyright (c) 2016-2018 DAC Technology Co., Ltd.
...
namespace ripple {
    ...
    {      
		...
		DacTx& dacTx = dynamic_cast<DacTx&>(transactor);
		...
		for (auto& eachTx : vecTxs)
		{
			...
			auto ret = TableStorageHandlePut(dacTx, uTxDBName, accountID, sTableName, lastLedgerSequence, tx.getTransactionID(), eachTx);
	...
	TER TableStorage::TableStorageHandlePut(DacTx& transactor,uint160 uTxDBName, AccountID accountID,std::string sTableName,uint32 lastLedgerSequence,uint256 txhash, STTx const & tx)
    {
        std::lock_guard<std::mutex> lock(mutexMap_);
