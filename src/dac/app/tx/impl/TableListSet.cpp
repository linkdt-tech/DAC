...
#include <dac/app/tx/impl/Tuning.h>
#include <dac/app/tx/OperationRule.h>
...
namespace ripple {
            ...
            if (tablesle)  //table exist
            {
                ...
                case T_CREATE:
                {
					...
					// Check reserve and funds availability
					{
						...
						STAmount priorBalance = STAmount((*sleAccount)[sfBalance]).dac();
	...
	std::pair<TER, std::string> TableListSet::dispose(TxStore& txStore, const STTx& tx)
	{
		...
		return DacTx::dispose(txStore, tx);
	}
