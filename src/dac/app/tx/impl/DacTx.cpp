...
namespace ripple {
	...
	DacTx::DacTx(ApplyContext& ctx)
		: Transactor(ctx)
	{

	}

	TER DacTx::preflight(PreflightContext const& ctx)
	{
		const STTx & tx = ctx.tx;
		Application& app = ctx.app;
		auto j = app.journal("preflightDac");
	...
	TER DacTx::preclaim(PreclaimContext const& ctx)
	{
		const STTx & tx = ctx.tx;
		Application& app = ctx.app;
		auto j = app.journal("preflightDac");
	...
	std::pair<TER, std::string> DacTx::dispose(TxStore& txStore, const STTx& tx)
	{
	...
	bool DacTx::canDispose(ApplyContext& ctx)
	{
	...
	std::pair<TxStoreDBConn*, TxStore*> DacTx::getTransactionDBEnv(ApplyContext& ctx)
	{
